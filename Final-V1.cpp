...
// a filled circle using triangles
...
// Draws the sun with rays + scaling animation
void drawSun() {
    glPushMatrix();
    glTranslatef(280, 220, 0);          // Sun er center e move koro
    glScalef(sunScale, sunScale, 1.0f); // Scale kora hoiche
    glTranslatef(-280, -220, 0);
...
void drawFarmHouse() {
    ...
    fillRect(-285, -32, -235, -20);
    fillRect(-278, -26, -242, -20);
...
void drawCowShed() {
    ...
    drawCow(-60, -15, 0.9, 0.9, 0.9);
    drawCow(50, -15, 0.5, 0.3, 0.1);
}
...
void update(int value) {
    ...
    // Sun scaling animation (NEW)
    if (sunGrow) sunScale += 0.003f;
    else sunScale -= 0.003f;
    if (sunScale >= 1.2f) sunGrow = false;
    if (sunScale <= 0.85f) sunGrow = true;
